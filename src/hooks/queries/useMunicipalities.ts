...
const getMunicipalitiesByTerritorialSecretaryId = async (
  territorialSecretaryId: string
) => {
  const response = await axios.get(
    `${config.apiUrl}/municipalities/territorial-secretary/${territorialSecretaryId}`
  );
  return response.data;
};
