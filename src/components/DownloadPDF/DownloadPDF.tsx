...
const DownloadPDF = ({ project }: { project: ProjectPayload<Project, 'projectBudget' | 'activities' | 'kpiInstances' | 'sector' | 'parish'> }) => {
    ...
    const parish = project.parish?.name || 'Todo el territorio'
    const municipality = project.parish?.municipality?.name || 'Todo el territorio'
    const startDate = project.initialDate
    const endDate = project.finalDate
    const territorialSecretary = project.parish?.municipality?.territorialSecretary?.name || 'Todo el territorio'
    const community = project.sector?.name || 'Todo el territorio'
    const circuit = project.sector?.communityCircuit?.name || 'Todo el territorio'
